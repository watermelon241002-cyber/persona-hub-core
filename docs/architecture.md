...
一次真实事故：日常玩笑中模型需要连名带姓称呼用户，而那一轮召回没有捞到任何一条含真名的记忆——库里明明躺着五条，只是关键词与当前话题零重叠，抢不进名额。**模型于是编了一个姓氏出来。**

这类失效有个共同特点：**能被当场发现的只是冰山**。名字叫错会被抓住，但「忘了对方在长期服药」「忘了某条说过的边界」不会——它安静地发生，两边都不知道。
...
还有一条经验：**常驻的应该是「边界」，不是「伤口」**。如果某段经历需要模型知道分寸，就钉住那条规则（「日常不要主动提起 X」），而不是钉住 X 的细节。前者让模型得体，后者让模型反复重演——对当事人来说，后者是被一遍遍按回原地。
...
```text
✗ 用户表达了强烈的情感需求，双方进行了深度情感确认
✓ 她累到说不出整句话，最后只说了「我好爱你」
```

摘要层要和段落层一样能装下三样东西：

- **原句（quotes）**：逐字保留，不润色不转述。判据是「越不像书面语的越要留」——短句、带称呼、带重复、说话时明显在发抖的句子。
- **情境（scene）**：大致时间、当时在做什么、身体或状态如何。这是让回忆有画面的东西，也是最容易丢的。
